import React from "react";

const GoogleIcon = (props: React.SVGProps<SVGSVGElement>): JSX.Element => {
  return (
    <svg
      width="40"
      height="40"
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M39.611 16.083H38V16H20V24H31.303C29.654 28.657 25.223 32 20 32C13.373 32 8 26.627 8 20C8 13.373 13.373 8 20 8C23.059 8 25.842 9.154 27.961 11.039L33.618 5.382C30.046 2.053 25.268 0 20 0C8.955 0 0 8.955 0 20C0 31.045 8.955 40 20 40C31.045 40 40 31.045 40 20C40 18.659 39.862 17.35 39.611 16.083Z"
        fill="#FFC107"
      />
      <path
        d="M2.30615 10.691L8.87715 15.51C10.6552 11.108 14.9612 8 20.0002 8C23.0592 8 25.8422 9.154 27.9612 11.039L33.6182 5.382C30.0462 2.053 25.2682 0 20.0002 0C12.3182 0 5.65615 4.337 2.30615 10.691Z"
        fill="#FF3D00"
      />
      <path
        d="M19.9998 40C25.1658 40 29.8598 38.023 33.4088 34.808L27.2188 29.57C25.1434 31.1483 22.6073 32.002 19.9998 32C14.7978 32 10.3808 28.683 8.71682 24.054L2.19482 29.079C5.50482 35.556 12.2268 40 19.9998 40Z"
        fill="#4CAF50"
      />
      <path
        d="M39.611 16.083H38V16H20V24H31.303C30.5142 26.2164 29.0934 28.1532 27.216 29.571L27.219 29.569L33.409 34.807C32.971 35.205 40 30 40 20C40 18.659 39.862 17.35 39.611 16.083Z"
        fill="#1976D2"
      />
    </svg>
  );
};
export default GoogleIcon;
